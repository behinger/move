...
function [tmpall, freqs, timesout, itcvals] = timefreq_par(data, srate, varargin)
...
if g.cycles(1) == 0
    ...
    elseif 0
        ...
        % apply filters
        % -------------
        verboseprintf(g.verbose, 'Parrallel Processing time point (of %d):',length(g.timesout));      
        
        if matlabpool('size') == 0 % checking to see if my pool is already open
    matlabpool open 4
        end
tic
tmpall = nan(length(g.win),length(g.indexout),size(g.win{1},2));                
        parfor index = 1:length(g.indexout)
            par_tmpall = nan(length(g.win),size(g.win{1},2));                
            wav = g.win{1}; 
            ...
            for freqind = 1:length(g.win);
                ...
                tmpX = sum(wav .* tmpX);
                par_tmpall(freqind,:) = tmpX;
            end;
            tmpall(:,index,:) = par_tmpall(:,:);
        end;
        matlabpool close
        toc
    end;
