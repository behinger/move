% eegplot_keyboard() - Scroll (horizontally and/or vertically) through multichannel data.
%             Allows vertical scrolling through channels and manual marking 
%             and unmarking of data stretches or epochs for rejection.
% %%% BEHINGER VERSION WITH KEYBOARD SUPPORT %%%%
% Usage: 
%           >> eegplot_keyboard(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
...
function [outvar1] = eegplot_keyboard(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
  ...
  % ---------------------------------
  defdowncom   = 'eegplot_keyboard(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'eegplot_keyboard(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'eegplot_keyboard(''defupcom'',     gcbf);';
  defctrldowncom = 'eegplot_keyboard(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      eegplot_keyboard('setelect', g.eloc_file, ax1);
  end;
...
% Five move buttons: << < text > >> 
  ...
  u(1) = uicontrol('Parent',figh, ...
	...
	'Tag','Pushbutton1',...
	'string','<<',...
	'Callback','eegplot_keyboard(''drawp'',1)');
  u(2) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position', posbut(2,:), ...
	'Tag','Pushbutton2',...
	'string','<',...
	'Callback','eegplot_keyboard(''drawp'',2)');
  u(5) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eegplot_keyboard(''drawp'',0);' );
  u(3) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position',posbut(3,:), ...
	'Tag','Pushbutton3',...
	'string','>',...
	'Callback','eegplot_keyboard(''drawp'',3)');
  u(4) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position',posbut(4,:), ...
	'Tag','Pushbutton4',...
	'string','>>',...
	'Callback','eegplot_keyboard(''drawp'',4)');
...
% Text edit fields: ESpacing
  ...
  u(6) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eegplot_keyboard(''draws'',0);' );
...
% ESpacing buttons: + -
  u(7) = uicontrol('Parent',figh, ...
	...
	'string','+',...
	'FontSize',8,...
	'Callback','eegplot_keyboard(''draws'',1)');
  u(8) = uicontrol('Parent',figh, ...
	...
	'string','-',...
	'FontSize',8,...
	'Callback','eegplot_keyboard(''draws'',2)');
...
cb_envelope = ['g = get(gcbf,''userdata'');'...
    ...
    'eegplot_keyboard(''drawp'',0);clear g;'];
...
u(22) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'eegplot_keyboard(''drawlegend'', gcbf)');
  end;
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eegplot_keyboard(''noui'');');
  ...
  m(11) = uimenu('Parent',m(1),'Label','Data select/mark', 'tag', 'displaywin', ...
          ...
          uimenu('Parent',m(11),'Label','Hide marks','Callback', ...
            ...
            'set(gcbf, ''userdata'', g);' ...
            'eegplot_keyboard(''drawb''); clear g;'] )
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'eegplot_keyboard(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','eegplot_keyboard(''title'')')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','eegplot_keyboard(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','eegplot_keyboard(''winelec'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','eegplot_keyboard(''loadelect'');')
  
  % Zooms %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Zoom off/on');
  commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); eegplot_keyboard(''''zoom'''', gcbf, 1);'' ]);' ...
  ...
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_keyboard(''drawp'', 0);'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_keyboard(''drawp'', 0);'];
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_keyboard(''emaxstring'');']; % JavierLC      
  comeventleg  = [ 'eegplot_keyboard(''drawlegend'', gcbf);'];
...
%  set(figh, 'busyaction', 'cancel');
  ...
  eegplot_keyboard('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eegplot_keyboard('scaleeye', 'off', gcf);
  end;
  
  eegplot_keyboard('drawp', 0);
  eegplot_keyboard('drawp', 0);
  if g.dispchans ~= g.chans
  	   eegplot_keyboard('zoom', gcf);
  end;  
  eegplot_keyboard('scaleeye', [], gcf);
...
%        keyboard;  
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		eegplot_keyboard( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   eegplot_keyboard('updateslider', fig);
   eegplot_keyboard('drawp',0);	
   eegplot_keyboard('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	eegplot_keyboard('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'noui'
      if ~isempty(varargin)
          eegplot_keyboard( varargin{:} ); fig = gcf;
      else 
   ...
   case 'zoom' % if zoom
      ...
      % ------------------------------
      if exist('p2') == 1
          set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); eegplot_keyboard(''zoom'', gcbf, 1);' ]);
  ...
  case 'defupcom'
    ...
    set(fig,'UserData', g);
    eegplot_keyboard('drawp', 0);
    if strcmp(g.mocap,'on'), show_mocap_for_eegplot_keyboard(g.winrej); g.winrej = g.winrej(end,:); end; % nima
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                eegplot_keyboard('drawp', 0);  % redraw background
            end;
        end;
    end;      
   otherwise
      error(['Error - invalid eegplot_keyboard() parameter: ',data])
  end  
end
...
function keyPress(src, e)
     switch e.Key
         case 'comma'
             eegplot_keyboard('drawp',1);
             
         case 'period'
             eegplot_keyboard('drawp',4);
         end
