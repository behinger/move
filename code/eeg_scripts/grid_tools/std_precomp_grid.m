% std_precomp_grid() - Precompute measures (ERP, spectrum, ERSP, ITC) for channels in a  
...
% Usage:    
% >> [ALLEEG,STUDY] = std_precomp_grid(STUDY, ALLEEG, chanorcomp, 'key', 'val', ...);
%
% Required inputs:
...
%
% Example:
%   >> [STUDY ALLEEG] = std_precomp_grid(STUDY, ALLEEG, { 'cz' 'oz' }, 'interpo', ...
...
function [ STUDY, ALLEEG ] = std_precomp_grid(STUDY, ALLEEG, chanlist, datasetIDX, varargin)
    
    if nargin < 2
        help std_precomp_grid;
        return;
    end;
    ...
    g = finputcheck(varargin, { 'erp'         'string'  { 'on','off' }     'off';
                                ...
                                'erpparams'         'cell'    {}                 {};
                                'erpimparams'       'cell'    {}                 {};
                                'erspparams'        'cell'    {}                 {}}, 'std_precomp_grid');
    ...
    if strcmpi(g.scalp, 'on')
        for index = datasetIDX% 1:length(STUDY.datasetinfo)
    ...
    if strcmpi(g.ersp, 'on') | strcmpi(g.itc, 'on')
        ...
        % ------------------------
        guimode = 'guion';
%              powBaseIndx = find(cellfun(@(x) strcmp(x,'powbase'),g.erspparams));
%         tmp = g.erspparams{(powBaseIndx)+1};
%         g.erspparams(powBaseIndx+1) = {tmp(1,:)};
...
%         g.erspparams(end+1) = {'powbase'};
%         g.erspparams(end+1) = {tmp};
       ...
       [ tmpX tmpt tmpf g.erspparams ] = std_ersp(ALLEEG(1), 'channels', 1, 'type', type, 'recompute', 'on', 'getparams', 'on', 'savetrials', g.savetrials, g.erspparams{:});
        ...
        for index = datasetIDX %1:length(STUDY.design(g.design).cell) % DATASETIDX COMES FROM THE GRID!
            desset = STUDY.design(g.design).cell(index);
            if strcmpi(computewhat, 'channels')
                [tmpchanlist opts] = getchansandopts(STUDY, ALLEEG, chanlist, desset.dataset, g);
                fprintf('Computing Channels')
