...
if ~isempty(strfind(setname, whatCheck)) && strcmp(res.overwrite,'off')
    ...
    if strcmp(res.silent,'off') && strcmp(input([whatCheck 'already found, do it again (y/n)? '],'s'),'y')
        if strcmp(whatCheck,'Clean') || strcmp(whatCheck,'Noisychannel') || strcmp(whatCheck,'Trialclean')
            error('Active EEG file already cleaned. Please reload in order to continue cleaning!')
        end
