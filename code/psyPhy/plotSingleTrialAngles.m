%% Plots single angles for single sessions
flags = mv_check_folderstruct
p = mv_generate_paths(flags.path{1}); % get the filepaths
...
for trial = 1:6
    ...
    plot(sT.EndPosX(trial), sT.EndPosZ(trial), 'o', 'Color', 'c');
%     plot(sT.EndPosStepX(trial), sT.EndPosStepZ(trial), 'x', 'Color', 'b');
%     plot(sT.EndPosArrX(trial), sT.EndPosArrZ(trial), 'x', 'Color', 'm');
